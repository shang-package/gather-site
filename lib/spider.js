...
function Spider(requestConfig, parseConfig, proxy, defaultRequest) {
  ...
  // 重试之前输出日志
  this.logRetryFun = this.requestConfig.logRetryFun || function(err, nu, url) {
    console.warn('url: ' + url + '; 第 ' + nu + ' 次错误');
  };

  // 删除request不需要的参数
  delete this.requestConfig.retryDelay;
  delete this.requestConfig.retryStrategy;
  delete this.requestConfig.logRetryFun;
    ...
    that.request(config)
      ...
      .catch(function(e) {
        if(that.retryStrategy(e)) {
          that.logRetryFun(e, deferred.nu, config.url);
    ...
    return function(rule, index) {
      for(var k in baseExtractRule) {
        ...
        if(baseExtractRule[k].isNeed && !rule[k]) {
          valid = false;
          console.warn('siteInfo.extract_rules[' + index + '] need attr: ', k);
        }
      }
    };
  }
