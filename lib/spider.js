...
function Spider(requestConfig, parseConfig, proxy) {
  this.requestConfig = extend({
    retryDelay: 0,
    retryStrategy: retryStrategy.all
  }, requestConfig);

  this.parseConfig = parseConfig;
  this.proxy = proxy || proxyPool.getProxy(false, true);
  this.cache = {};
  this.content = null;
  ...
  // 重试间隔, 默认为0
  this.retryDelay = this.requestConfig.retryDelay;
  // 重试机制
  this.retryStrategy = this.requestConfig.retryStrategy;

  // 使用 request.defaults 定义的request
  this.request = this.requestConfig.request || request;
  delete this.requestConfig.request;

  // 默认 encoding 为 null 获取内容,方便转码
  if(this.encodingCheck && parseConfig) {
    parseConfig.encoding = parseConfig.encoding || this.requestConfig.encoding;
...
Spider.prototype.download = function() {
  ...
  if(this.requestConfig.hasOwnProperty('proxy')) {
    deferred.nu = this.proxy.tryRange[1];
  }
  else {
  ...
  function proxyGet(deferred) {
    ...
    if(!config.proxy) {
      config.proxy = that.proxy.getOne(deferred.nu);
    }
